...
jQuery(document).ready(function( $ ) {
  ...
  var swiper2 = new Swiper(".specialities-swiper", {
 slidesPerView: 5,
 slidesPerGroup: 5, 
  spaceBetween: 20, 
  grid: {
  ...
  breakpoints: {
	...
	$('.nav-item, .nav__item').on('click', function() {
	...
	var whsh = window.location.hash;
	
	function open_tab(hsh){
		
		//console.log(hsh.startsWith('#tab-'));
		console.log(hsh);
		
		
		if (hsh && hsh.startsWith('#tab-')) {
			var id = hsh.substring(5); 
			$('.nav-item, .nav__item').removeClass('active');
			$('.nav-item[data-id='+id+']').addClass('active');
			//$('.nav__item[data-id='+id+']').addClass('active');
			
			
			if ($('.nav__item').length) {
				setTimeout(function(){
					$('.nav__item[data-id='+id+']').click();
				},500);
			}
			
			
			$('.block_content, .tab_block').removeClass('active');
			
			$('#' + id).addClass('active');
			if ($('#solution-tabs').length) {
				$('html, body').animate({
					scrollTop: $('#solution-tabs').offset().top
				}, 600);
				return;
			}
			if ($('.acute_community').length) {
				$('html, body').animate({
					scrollTop: $('#referrals').offset().top
				}, 600);
				return;
			}
			if ($('#latest').length) {
	...
	$('.submenu-link').click(function(){
		let hsh = '#' + $(this).attr('href').split('#')[1];
		open_tab(hsh);
	});
...
jQuery(document).ready(function( $ ) {
	...
	$(document).on('click', '.submenu-link', function(e){
		setTimeout(function(){
			...
			$('.header__menu.show').removeClass('show');
			$('.mobile-menu-icon .bar').removeClass('close');
		}, 1000);
	});
...
document.querySelector('.menu-item-123 a')?.setAttribute('target', '_blank');
